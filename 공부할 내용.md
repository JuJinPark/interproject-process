...
- id 소대문자 고치기 
- 테스트시 개별 테스트후 롤백? 모든 테스트후 롤백?(delete?필요?))
- mysql8 으로도 roll back 가능?
- @transactional 메서드에 단다
- object mapper 변경 및 테스트
- map struct 구동 원리
- object mapper 인터페이스화
- cross origin issue 어노테이션으로 하기
- api만들기 계속
...
변경 테스트 floor new floor vs getbyid 한 걸 insert후 update 해볼거
lazy 로딩후 조인어덯게 쿼리로 하는지 검사


5.질문
테스트 코드 구성
@valid on condition 되나?

@requestbody as not object
빌더 패턴 안쓴이유- 생성자가 더 추가될일이 없을거같고
보통 사용자가 객체생성르 쉽게 하고 가독성 측면인데
객체자체가 자주 생성하고 optional로 줄 변수가 있어야 된다고 생각한다
하지만 이건 엔티티 객치자체를 저 메서드로만 하게 강제하기위해 작성

서비스단에서 exception test? 이미 만약 db에 존재하는 id 가있으면 터무니없느 id 어떻게 알아서 생성할까?
request dto update with employee 로 궅이 분리햇는데 필요 해보이나? 
서비스 테스트 분리 하기 getAll에 다른 기존 db 데이터 가 보일수도 잇다 이때는 그냥 방금넣은게 잇는지만 체크?
하드코딩을 띄어내고 싶다 id 가 업슨 상화엥서 어떻게 하면 좋을까?
아예 rest api에서 필요없느거는 서비스 자체도 안만드는게 낫나? 예 floor 초기에 데이터베이스에 그냥 밀어넣느다. 아니 고려해서 만들어야 되나?
