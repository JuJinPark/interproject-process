...
- 스프링 멀티 스레드 만약 여러 사람이 이용한느 경우는 머지 
  ...
  - cookie 와 redireciotn 결정해주는 곳 서비스 or  controller
  - controller redirection 결정할때 그럼 response를 메서드안에 담냐 아니면 뺴냐
  - exception 처러로직 및 처리 특히 ouath,get accesstoken 할때 외부 라이브러리와의 분리를 위해 자체적인 exception을 생성해서 interface를 명명하고 던지는게 더 나은 방식맞나?
  - social login github gmail 이메일이 같다면??? provider 추가해서 해주어야하나?