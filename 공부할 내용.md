...
- 실제 db에 테스트 할떄는 @transactional 후 롤백이 적용안된느듯하다 inmemmory 에서 테스트 한번더 하기 바람
...
- foreign key 이르 주어야 하나?
- 테스트 단위난 테스트 구조

- spring security 로 로그인 구현 하는 방향

- private setter vs this.로 변환
...
하지만 외부 api 면 entity를 반환 하기보다는 dto 가 안전 내부면(서버 및 화면 모두 통제가능 상황) entity 사용 
oslc 사용시 controller 바로 repository 호출해서 간단한 건 처리 가능 
cascade 와 같은 정책은 아직;;
